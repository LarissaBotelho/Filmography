...
function favoritar(id) {
  var detailMovie = allMovies.filter(movie =>{
    if (movie.id == id){
      return movie
    }
  })
    console.log(detailMovie)
  var idStorage = localStorage.getItem('favMovies')
  idStorage = JSON.parse(idStorage)
  if (idStorage && idStorage.length > 0) {
      idStorage.push(detailMovie[0])
      localStorage.setItem('favMovies', JSON.stringify(idStorage));
  }
  else {
    var idFilmes = []
    idFilmes.push(detailMovie[0])
    localStorage.setItem('favMovies', JSON.stringify(idFilmes));
